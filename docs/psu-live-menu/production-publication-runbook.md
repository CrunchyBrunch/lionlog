...
`RELEASE_MANIFEST_PATH` is the only accepted release-manifest environment variable in the final verifier, policy shell, and Pages adapter. It must name the regular, non-symlink `release-manifest.json` at the canonical bundle location; an adjacent filename, path alias, changed file identity, or symlink is rejected. The adapter reads those exact bytes through one file handle, validates their digest and the authoritative manifest schema, release identity, provenance/approval policy, normalized menu evidence, complete inventory, deterministic tar, target, source/recovery/staged identities, and retained approval summary, then carries that validated manifest forward instead of reopening a neighboring file. The same exact-file verification runs again after OIDC. Missing paths, the former name, unreadable or unsupported manifests, semantic mismatches, altered tar bytes, and approval-tuple drift fail before any repository publication-ledger or Pages POST. New failures at this boundary retain `pre-submission` evidence with `not-submitted`; they do not need the historical reconciliation exception.
