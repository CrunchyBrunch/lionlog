...
`./menu-data/v1/catalog.json` is resolved against the document base URL, so root hosting and `/lionlog/` hosting use the same build. Snapshot URLs resolve relative to the catalog and must remain same-origin. The service worker deliberately excludes `menu-data/`; validated IndexedDB state owns menu-data offline behavior while the service worker owns the application shell.
