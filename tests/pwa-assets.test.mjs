...
test("live and Pages workflows are explicit, bounded, and ordinary CI cannot invoke ingestion or deployment", async () => {
  ...
  assert.match(deploymentWorkflow, /expected_predecessor_release_id/);
  assert.match(deploymentWorkflow, /approval_expires_at/);
  ...
  assert.match(deploymentWorkflow, /collect-pages-attempt-history\.ts/);
  assert.match(deploymentWorkflow, /select-browser-verification-context\.ts/);
  ...
  assert.match(deploymentWorkflow, /group: github-pages-production/);
  assert.doesNotMatch(deploymentWorkflow, /group: lionlog-pages-production/);
  assert.match(deploymentWorkflow, /summary_artifact_digest: \$\{\{ steps\.summary-digest\.outputs\.digest \}\}/);
  assert.equal((deploymentWorkflow.match(/artifact-digest\.ts --expected="\$SUMMARY_DIGEST" --file=/g) ?? []).length, 2);
  assert.match(deploymentWorkflow, /RAW_DIGEST: \$\{\{ steps\.summary-upload\.outputs\.artifact-digest \}\}/);
  ...
  assert.doesNotMatch(deploymentWorkflow, /partial_approval|pre_submission_failure_approval|first-release-recovery/);
